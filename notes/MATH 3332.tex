...
\section{Linear approximation and Differentiation}
    ...
    \begin{thm}
        Let $f:H \to \mathbb{R}$. Assume that $f$ is differentiable at $\mathbf{x} \in \mathbb{R}^{n}$. For any $\mathbf{u} \in \mathbb{R}^{n}$ that satisfies $\norm{\mathbf{u}} = 1$,
        \begin{equation*}
            \inprod{\nabla f(\mathbf{x})}{\mathbf{u}} = \left.\odv*{f(\mathbf{x} + t\mathbf{u})}{t}\right|_{t=0}.
        \end{equation*}
    ...
    \begin{defn}
        ...
        \end{equation*}
    \end{defn} 
    We will continue discussing this differentiation in the next chapter. Read Appendix \ref{Case Study D: Linear Regression} (Linear Regression), \ref{Case Study E: Kernel Ridge Regression} (Kernel Ridge Regression), and \ref{Case Study F: Linear Classification} (Linear Classification) to see the case studies for Chapter \ref{Chapter 4: Linear Functions and Differentiation}.
...
\chapter{Linear Regression}
    ...
    \begin{enumerate}
        ...
        However, in practice, we may also have cases when $N << n$. For example, in image recognition. Therefore, we need to perform some regularizations to further shrink the set of candidate functions. We will discuss this later.
    \end{enumerate}
