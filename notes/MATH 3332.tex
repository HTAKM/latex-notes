...
\chapter{Gradient Descent}
    ...
    Suppose we have a function $f: H \to \mathbb{R}$. We aim to find:
    \begin{equation*}
        \mathbf{x}^{*} = \argmin_{\mathbf{x} \in H} f(\mathbf{x}),
    \end{equation*}
    where $H$ is a Hilbert space. How can we find the solution numerically? Assume we have an estimate $\mathbf{x}^{(k)}$ of $\mathbf{x}^{*}$. How can we find a better estimate $\mathbf{x}^{(k+1)}$? We can use an affine approximation:
    \begin{equation*}
    ...
    \begin{defn}
        The \textbf{Gradient Descent algorithm} is a method to minimize a function $f: H \to \mathbb{R}$. The steps of the algorithm are as follows:
        \begin{itemize}
