...
\date{
	...
	Last small update: December 22, 2025
}
\begin{document}
\maketitle
    This lecture note is based on the HKUST MATH 3332 lecture notes by Prof. YE, Guibo (Fall 2025-26). The topics are grouped into chapters instead of lectures, and case studies are moved to separate sections.\\
    Some proofs are written by myself, as they are either not explained clearly or not found in the lecture notes. These may contain errors. If you notice any, you likely possess a strong understanding of the topic or a keen eye for detail. ;)\\
    This course requires prerequisites in multivariable calculus (MATH 2011 and MATH 2023 for HKUST students) and linear algebra (MATH 2111, MATH 2121, and MATH 2131 for HKUST students).\\
...
\chapter{Newton's Method}
    \label{Case Study I: Newton's Method}
    This case study assumes that you have already read Chapter \ref{Chapter 5: Linear Transformations or Linear Operators} and Appendix \ref{Case Study H: Gradient Descent}.

    Let $H$ be a Hilbert space. Suppose we have a function $f: H \to \mathbb{R}$. We aim to find the solution of:
    \begin{equation*}
        \mathbf{x}^{*} = \argmin_{\mathbf{x} \in H} f(\mathbf{x}).
    \end{equation*}
    Given an estimate $\mathbf{x}^{(k)}$ of $\mathbf{x}^{*}$, instead of using an affine approximation as in Appendix \ref{Case Study H: Gradient Descent}, we can use the second-order Taylor expansion:
    \begin{equation*}
        f(\mathbf{x}) \approx f(\mathbf{x}^{(k)}) + \inprod{\nabla f(\mathbf{x}^{(k)})}{\mathbf{x} - \mathbf{x}^{(k)}} + \frac{1}{2}\inprod{\nabla^{2} f(\mathbf{x}^{(k)})(\mathbf{x}-\mathbf{x}^{(k)})}{\mathbf{x}-\mathbf{x}^{(k)}}
    \end{equation*}
    if $f$ and $\nabla f$ are differentiable at $\mathbf{x} \in H$ and $\snorm{\mathbf{x}^{(0)} - \mathbf{x}^{*}}$ is small. We may denote $F_{k}: H \to \mathbb{R}$ as:
    \begin{equation*}
        F_{k}(\mathbf{x}) = f(\mathbf{x}^{(k)}) + \inprod{\nabla f(\mathbf{x}^{(k)})}{\mathbf{x} - \mathbf{x}^{(k)}} + \frac{1}{2}\inprod{\nabla^{2} f(\mathbf{x}^{(k)})(\mathbf{x}-\mathbf{x}^{(k)})}{\mathbf{x}-\mathbf{x}^{(k)}}.
    \end{equation*}
    Therefore, by the first-order optimality condition, our problem can be rewritten as:
    \begin{equation*}
        \mathbf{x}^{(k+1)} = \argmin_{\mathbf{x} \in H} F_{k}(\mathbf{x}) \iff \nabla F_{k}(\mathbf{x}^{(k+1)}) = \mathbf{0}.
    \end{equation*}
    Since $\nabla^{2} f(\mathbf{x})$ is linear and self-adjoint,
    \begin{align*}
        \nabla F_{k}(\mathbf{x}) &= \nabla f(\mathbf{x}^{(k)}) + \frac{1}{2}((\nabla^{2} f(\mathbf{x}^{(k)}))^{*}(\mathbf{x}-\mathbf{x}^{(k)}) + \nabla^{2} f(\mathbf{x}^{(k)})(\mathbf{x}-\mathbf{x}^{(k)}))\\
        &= \nabla f(\mathbf{x}^{(k)}) + \nabla^{2} f(\mathbf{x}^{(k)})(\mathbf{x}-\mathbf{x}^{(k)}),\\
        \mathbf{0} = \nabla F_{k}(\mathbf{x}^{(k+1)}) &= \nabla f(\mathbf{x}^{(k)}) + \nabla^{2} f(\mathbf{x}^{(k)})(\mathbf{x}^{(k+1)}-\mathbf{x}^{(k)}),\\
        \mathbf{x}^{(k+1)} &= \mathbf{x}^{(k)} - (\nabla^{2} f(\mathbf{x}^{(k)}))^{-1}\nabla f(\mathbf{x}^{(k)}).
    \end{align*}
    Therefore, we have the following method:
    \begin{defn}
        The \textbf{Newton's method} is a technique to minimize a twice-differentiable function $f: H \to \mathbb{R}$. The steps of the algorithm are as follows:
        \begin{itemize}
            \item[0:] Initialize $\mathbf{x}^{(0)}$.
            \item[1:] For $k = 0, 1, \cdots$, update $\mathbf{x}^{(k+1)}$ by:
            \begin{equation*}
                \mathbf{x}^{(k+1)} = \mathbf{x}^{(k)} - (\nabla^{2} f(\mathbf{x}^{(k)}))^{-1} \nabla f(\mathbf{x}^{(k)}).
            \end{equation*}
            \item[] Repeat until convergence is achieved.
    ...
    \end{defn}
    \begin{rem}
        Newton's method usually requires fewer iterations than gradient descent if it converges, because the second-order approximation $F_{k}$ is more accurate than the first-order approximation used in gradient descent.
    \end{rem}
    \newpage

    There are some main concerns in Newton's method:
    \begin{itemize}
        \item[1.] It converges to $\mathbf{x}^{*}$ only if $\snorm{\mathbf{x}^{(0)} - \mathbf{x}^{*}}$ is sufficiently small. We can add a step size to the algorithm, similar to gradient descent.
        \begin{defn}
            The \textbf{Newton's method with a step size} is a technique to minimize a twice-differentiable function $f: H \to \mathbb{R}$. The steps of the algorithm are as follows:
            \begin{itemize}
                \item[0:] Initialize $\mathbf{x}^{(0)}$.
                \item[1:] For $k = 0, 1, \cdots$,
                \begin{enumerate}
        ...
        \begin{equation*}
            \mathbf{x}^{(k+1)} = \argmin_{\mathbf{x} \in H} F_{k}(\mathbf{x}) \text{ such that } \snorm{\mathbf{x}-\mathbf{x}^{(k)}} \leq \alpha_{k}\snorm{\nabla f(\mathbf{x}^{(k)})},
        \end{equation*}
        where $\alpha_{k} > 0$ is the step size. The derivation is omitted, but we obtain the following algorithm:
        \begin{defn}
            The \textbf{damped Newton's method} is a technique to minimize a twice-differentiable function $f: H \to \mathbb{R}$. The steps of the algorithm are as follows:
            \begin{itemize}
                \item[0:] Initialize $\mathbf{x}^{(0)}$.
                \item[1:] For $k = 0, 1, \cdots$,
                \begin{enumerate}
                    \item Choose the regularization parameter $\lambda_{k}$.
                    \item Update $\mathbf{x}^{(k+1)}$ by:
                    \begin{equation*}
                        \mathbf{x}^{(k+1)} = \mathbf{x}^{(k)} - (\nabla^{2} f(\mathbf{x}^{(k)}) + \lambda_{k}\mathbf{I})^{-1} \nabla f(\mathbf{x}^{(k)}).
                    \end{equation*}
                \end{enumerate}
                \item[] Repeat until convergence is achieved.
            \end{itemize}
        ...
        \item[2.] The computation of the inversion of the Hessian is too expensive. We need to approximate $\nabla^{2} f(\mathbf{x}^{(k)})$ by some simple linear operators $\mathbf{B}_{k}$ so that finding $\mathbf{B}_{k}^{-1}$ is much easier than finding $(\nabla^{2} f(\mathbf{x}^{(k)}))^{-1}$. Therefore,
        \begin{equation*}
            \mathbf{x}^{(k+1)} = \mathbf{x}^{(k)} - \mathbf{B}_{k}^{-1} \nabla f(\mathbf{x}^{(k)}).
        \end{equation*}
        \begin{defn}
            The \textbf{quasi-Newton method} is a technique to minimize a twice-differentiable function $f: H \to \mathbb{R}$ without the need to find the inverse of the Hessian. The steps of the algorithm are as follows:
            \begin{itemize}
                \item[0:] Initialize $\mathbf{x}^{(0)}$.
                \item[1:] For $k = 0, 1, \cdots$,
                \begin{enumerate}
                    \item Find the approximation $\mathbf{B}_{k}$ of $\nabla^{2} f(\mathbf{x}^{(k)})$.
                    \item Update $\mathbf{x}^{(k+1)}$ by:
                    \begin{equation*}
                        \mathbf{x}^{(k+1)} = \mathbf{x}^{(k)} - \mathbf{B}_{k}^{-1} \nabla f(\mathbf{x}^{(k)}).
                    \end{equation*}
        ...
        \end{defn}
        \begin{rem}
            If $\mathbf{B}_{k}$ is diagonal, then it is a scaled gradient descent.
        \end{rem}
        There are many methods to find $\mathbf{B}_{k}$, such as:
        \begin{enumerate}
            \item Broyden-Fletcher-Goldfarb-Shanno algorithm (BFGS),
            \item Davidon-Fletcher-Powell formula (DFP).
        \end{enumerate}
        The inverse is calculated using the Sherman-Morrison formula.
    \end{itemize}

\chapter{Deep Neural Network Training in Deep Learning}
    \label{Case Study J: Deep Neural Network Training in Deep Learning}
    This case study assumes that you have already read Chapter \ref{Chapter 5: Linear Transformations or Linear Operators} and Appendices \ref{Case Study E: Kernel Ridge Regression} and \ref{Case Study H: Gradient Descent}.

    Suppose that we are given $N$ input-output pairs:
    \begin{equation*}
        (\mathbf{x}^{(1)}, y_{1}), \dots, (\mathbf{x}^{(N)}, y_{N})
    \end{equation*}
    where $\mathbf{x}^{(i)} \in \mathbb{R}^{n}$ and $y_{i} \in \mathbb{R}$ for $i = 1, \dots, N$. Previously, we used different methods to find the function $f$ that predicts the outputs:
    \begin{enumerate}
        \item Linear regression: $f$ is an affine function.
        \item Kernel ridge regression: $f$ is a linear function in the feature domain.
    \end{enumerate}
    ...
    \begin{equation*}
        y = \sigma\left(\sum_{i=1}^{n}w_{i}x_{i}\right) = \sigma(\inprod{\mathbf{w}}{\mathbf{x}})
    \end{equation*}
    where $\mathbf{w} \in \mathbb{R}^{n}$ and $\sigma: \mathbb{R} \to \mathbb{R}$ is an activation function.
    \begin{figure}[h]
        \centering
        \begin{tikzpicture}[cnode/.style={draw=black, fill=#1, minimum width=1mm, circle}]
            \node[cnode=black, fill=white] (s) at (3, -3) {};
            \node[cnode=black, fill=white, label=0:$y$] (y) at (6, -3) {};
            \draw[->] (s) -- (y) node[above, midway] {$\sigma$};
            \node at (0, -4) {$\vdots$};
            \foreach \x in {1, ..., 4} {
                \pgfmathparse{\x<4 ? \x : "n"}
                \node[cnode=black, fill=white, label=180:$x_{\pgfmathresult}$] (x-\x) at (0,{-\x-\intcalcDiv{\x}{4}}) {};
                \draw[->] (x-\x) -- (s) node[above, midway] {$w_{\pgfmathresult}$};
            }
        \end{tikzpicture}
    \end{figure}
    ...
    \begin{figure}[h!]
        ...
        \begin{subfigure}[h]{.34\textwidth}
            \begin{equation*}
                y = \sum_{i=1}^{p} u_{i}\sigma(\inprod{\mathbf{w}_{i}}{\mathbf{x}}), \qquad \text{where } \mathbf{u} \in \mathbb{R}^{p}.
            \end{equation*}
        \end{subfigure}
        \begin{subfigure}[h]{.65\textwidth}
            \centering
            \begin{tikzpicture}[cnode/.style={draw=black, fill=#1, minimum width=1mm, circle}]
                \node[cnode=black, fill=white, label=0:$y$] (y) at (9, -3) {};
                \node at (0, -4) {$\vdots$};
                \node at (3, -4) {$\vdots$};
                \node at (6, -4) {$\vdots$};
                \foreach \x in {1, ..., 4} {
                ...
                \foreach \y in {1, ..., 4} {
                    \pgfmathparse{\y<4 ? \y : "p"}
                    \node[cnode=black, fill=white] (s-\y) at (3,{-\y-\intcalcDiv{\y}{4}}) {};
                    \node[cnode=black, fill=white] (a-\y) at (6,{-\y-\intcalcDiv{\y}{4}}) {};
                    \draw[->] (s-\y) -- (a-\y) node[above, midway] {$\sigma$};
                    \draw[->] (a-\y) -- (y) node[above, midway] {$u_{\pgfmathresult}$};
                    \foreach \x in {1, ..., 4} {
                        \draw[->] (x-\x) -- (s-\y);
                    }
                }
    ...
    \begin{align*}
        \mathbf{W} &= \begin{pmatrix}
            ...
            \vdots\\
            \mathbf{w}_{p}^{T}
        \end{pmatrix} \in \mathbb{R}^{p \times n}, & \mathbf{u} &\in \mathbb{R}^{p}, & \sigma(\mathbf{y}) &= \begin{pmatrix}
            \sigma(y_{1})\\
            \vdots\\
            \sigma(y_{p})
        \end{pmatrix}.
    ...
    \begin{align*}
        f_{\mathbf{W},\mathbf{u}}(\mathbf{x}^{(i)}) &\approx y_{i}, \qquad \text{for } i = 1, \dots, N, & f_{\mathbf{W},\mathbf{u}}(\mathbf{x}) &= \inprod{\mathbf{u}}{\sigma(\mathbf{W}\mathbf{x})}, \qquad \text{for } \mathbf{x} \in \mathbb{R}^{n}.
    \end{align*}
    We define $F_{i}(\mathbf{W}, \mathbf{u}) = (f_{\mathbf{W},\mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})^{2}$ and $F(\mathbf{W}, \mathbf{u}) = \sum_{i=1}^{N} F_{i}(\mathbf{W}, \mathbf{u})$. Our problem now is to solve:
    \begin{equation*}
        \min_{\substack{\mathbf{W} \in \mathbb{R}^{p \times n}\\\mathbf{u} \in \mathbb{R}^{p}}} \sum_{i=1}^{N} (f_{\mathbf{W},\mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})^{2} \iff \min_{\substack{\mathbf{W} \in \mathbb{R}^{p \times n}\\\mathbf{u} \in \mathbb{R}^{p}}} F(\mathbf{W}, \mathbf{u}) \iff \min_{\substack{\mathbf{W} \in \mathbb{R}^{p \times n}\\\mathbf{u} \in \mathbb{R}^{p}}} \sum_{i=1}^{N} F_{i}(\mathbf{W}, \mathbf{u}).
    \end{equation*}
    \begin{rem}
        $F(\mathbf{W}, \mathbf{u})$ is differentiable if $\sigma$ is differentiable.
    \end{rem}
    \begin{rem}
        $F(\mathbf{W}, \mathbf{u})$ is not convex in general.
    \end{rem}
    ...
    \end{rem}
    To find $\nabla_{\mathbf{W}} F_{i}(\mathbf{W}, \mathbf{u})$, let:
    \begin{align*}
        G_{1}(t) &= (t-y_{i})^{2}, \qquad \text{for } t \in \mathbb{R}, & G_{2}(\mathbf{W}) &= f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}), \qquad \text{for } \mathbf{W} \in \mathbb{R}^{p \times n}, & F_{i}(\mathbf{W}, \mathbf{u}) &= (G_{1} \circ G_{2})(\mathbf{W}).
    \end{align*}
    By the chain rule,
    \begin{equation*}
        \nabla_{\mathbf{W}}F_{i}(\mathbf{W}, \mathbf{u}) = G_{1}'(G_{2}(\mathbf{W})) \cdot \nabla G_{2}(\mathbf{W}) = 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i}) \cdot \nabla_{\mathbf{W}}f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}).
    \end{equation*}
    Again, let:
    \begin{align*}
        H_{1}(\mathbf{z}) &= \inprod{\mathbf{u}}{\mathbf{z}}, \qquad \text{for } \mathbf{z} \in \mathbb{R}^{p}, & H_{2}(\mathbf{W}) &= \sigma(\mathbf{W}\mathbf{x}^{(i)}), \qquad \text{for } \mathbf{W} \in \mathbb{R}^{p \times n}, & f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) &= (H_{1} \circ H_{2})(\mathbf{W}).
    \end{align*}
    By the chain rule,
    \begin{equation*}
    ...
    \end{equation*}
    By Theorem \ref{Chapter 5 (Theorem) Jacobian Matrix}, for any $\mathbf{z} \in \mathbb{R}^{p}$,
    \begin{align*}
        D \sigma(\mathbf{z}) &= \begin{pmatrix}
            \sigma'(z_{1}) & 0 & \hdots & 0\\
            0 & \sigma'(z_{2}) & \ddots & \vdots\\
            \vdots & \ddots & \ddots & 0\\
            0 & \hdots & 0 & \sigma'(z_{p})
        \end{pmatrix} = \diag(\sigma'(\mathbf{z})), & D (\sigma(\mathbf{W}\mathbf{x}^{(i)}))(\mathbf{V}) &= \diag(\sigma'(\mathbf{W}\mathbf{x}^{(i)}))V\mathbf{x}^{(i)}.
    \end{align*}
    By the definition of the adjoint operator, since $\inprod{\mathbf{u}}{\mathbf{v}} = \mathbf{v}^{T}\mathbf{u} = \Tr(\mathbf{u}\mathbf{v}^{T})$ for $\mathbf{u}, \mathbf{v} \in \mathbb{R}^{n}$,
    \begin{align*}
        \inprod{D(\sigma(\mathbf{W}\mathbf{x}^{(i)}))(\mathbf{V})}{\mathbf{z}} &= \inprod{\diag(\sigma'(\mathbf{W}\mathbf{x}^{*}))\mathbf{V}\mathbf{x}^{(i)}}{\mathbf{z}}\\
        &= \mathbf{z}^{T}\diag(\sigma'(\mathbf{W}\mathbf{x}^{*}))\mathbf{V}\mathbf{x}^{(i)}\\
        &= \Tr(\mathbf{x}^{(i)}\mathbf{z}^{T}\diag(\sigma'(\mathbf{W}\mathbf{x}^{*}))\mathbf{V}) = \inprod{\mathbf{V}}{\diag(\sigma'(\mathbf{W}\mathbf{x}^{*}))\mathbf{z}(\mathbf{x}^{(i)})^{T}}.
    \end{align*}
    Therefore, $(D(\sigma(\mathbf{W}\mathbf{x}^{(i)}))(\mathbf{V}))^{*}\mathbf{u} = \diag(\sigma'(\mathbf{W}\mathbf{x}^{*}))\mathbf{u}(\mathbf{x}^{(i)})^{T}$ and thus,
    \begin{equation*}
        \nabla_{\mathbf{W}} F_{i}(\mathbf{W}, \mathbf{u}) = 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i}) \cdot \nabla_{\mathbf{W}}f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)})\diag(\sigma'(\mathbf{W}\mathbf{x}^{*}))\mathbf{u}(\mathbf{x}^{(i)})^{T}.
    \end{equation*}
    \newpage

    To find $\nabla_{\mathbf{u}} F_{i}(\mathbf{W}, \mathbf{u})$, we can again let:
    \begin{align*}
        G_{1}(t) &= (t-y_{i})^{2}, \qquad \text{for } t \in \mathbb{R}, & G_{2}(\mathbf{u}) &= \inprod{\mathbf{u}}{\sigma(\mathbf{W}\mathbf{x}^{(i)})}, \qquad \text{for } \mathbf{u} \in \mathbb{R}^{p}, & F_{i}(\mathbf{W}, \mathbf{u}) &= (G_{1} \circ G_{2})(\mathbf{u}).
    \end{align*}
    By the chain rule,
    \begin{equation*}
        \nabla_{\mathbf{u}}F_{i}(\mathbf{W}, \mathbf{u}) = G_{1}'(G_{2}(\mathbf{u})) \cdot \nabla G_{2}(\mathbf{W}) = 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i}) \cdot \nabla_{\mathbf{u}}\left(\inprod{\mathbf{u}}{\sigma(\mathbf{W}\mathbf{x}^{(i)})}\right) = 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})\sigma(\mathbf{W}\mathbf{x}^{(i)})
    \end{equation*}

    We can extend the neural network into multi-layer neural network. Assume that there are $L$ hidden layers.
    \begin{equation*}
        f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}) = \inprod{\mathbf{u}}{\sigma(\mathbf{W}^{(L)} \cdots \sigma(\mathbf{W}^{(2)}\sigma(\mathbf{W}^{(1)})))} = \inprod{\mathbf{u}}{(\sigma \circ \mathbf{W}^{(L)} \circ \cdots \circ \sigma \circ \mathbf{W}^{(2)} \circ \sigma \circ \mathbf{W}^{(1)})(\mathbf{x})}
    \end{equation*}
    \begin{figure}[h]
        \centering
        \begin{tikzpicture}[cnode/.style={draw=black, fill=#1, minimum width=1mm, circle}]
            \node[cnode=black, fill=white, label=0:$y$] (y) at (16, -3) {};
            \foreach \i in {0, ..., 6} {
                \node at (\i * 2, -4) {$\vdots$};
            }
            \foreach \x in {1, ..., 4} {
                \pgfmathparse{\x<4 ? \x : "n"}
                \node[cnode=black, fill=white, label=180:$x_{\pgfmathresult}$] (x0-\x) at (0,{-\x-\intcalcDiv{\x}{4}}) {};
            }
            \node at (1, -5.5) {$\mathbf{W}^{(1)}$};
            \foreach \y in {1, ..., 4} {
                \node[cnode=black, fill=white] (s0-\y) at (2,{-\y-\intcalcDiv{\y}{4}}) {};
                \node[cnode=black, fill=white] (x1-\y) at (4,{-\y-\intcalcDiv{\y}{4}}) {};
                \foreach \x in {1, ..., 4} {
                    \draw[->] (x0-\x) -- (s0-\y);
                }
                \draw[->] (s0-\y) -- (x1-\y) node[above, midway] {$\sigma$};
            }
            \node at (5, -5.5) {$\mathbf{W}^{(2)}$};
            \foreach \y in {1, ..., 4} {
                \node[cnode=black, fill=white] (s1-\y) at (6,{-\y-\intcalcDiv{\y}{4}}) {};
                \node[cnode=black, fill=white] (x2-\y) at (8,{-\y-\intcalcDiv{\y}{4}}) {};
                \foreach \x in {1, ..., 4} {
                    \draw[->] (x1-\x) -- (s1-\y);
                }
                \draw[->] (s1-\y) -- (x2-\y) node[above, midway] {$\sigma$};
            }
            \foreach \x in {1, ..., 4} {
                \node at (9,{-\x-\intcalcDiv{\x}{4}}) {$\hdots$};
                \node[cnode=black, fill=white] (x3-\x) at (10,{-\x-\intcalcDiv{\x}{4}}) {};
            }
            \node at (11, -5.5) {$\mathbf{W}^{(L)}$};
            \foreach \y in {1, ..., 4} {
                \pgfmathparse{\y<4? \y : "p"}
                \node[cnode=black, fill=white] (s3-\y) at (12,{-\y-\intcalcDiv{\y}{4}}) {};
                \node[cnode=black, fill=white] (x4-\y) at (14,{-\y-\intcalcDiv{\y}{4}}) {};
                \foreach \x in {1, ..., 4} {
                    \draw[->] (x3-\x) -- (s3-\y);
                }
                \draw[->] (s3-\y) -- (x4-\y) node[above, midway] {$\sigma$};
                \draw[->] (x4-\y) -- (y) node[above, midway] {$u_{\pgfmathresult}$};
            }
        \end{tikzpicture}
    \end{figure}

    Let $\mathbf{W} = [\mathbf{W}^{(1)}, \mathbf{W}^{(2)}, \cdots, \mathbf{W}^{(L)}] \in \mathbb{R}^{n_{1} \times n} \times \mathbb{R}^{n_{2} \times n_{1}} \times \cdots \times \mathbb{R}^{n_{L} \times n_{L-1}}$. We aim to find $\mathbf{u} \in \mathbb{R}^{p}$ and $\mathbf{W}$ such that:
    \begin{equation*}
        f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) \approx y_{i}, \qquad \text{for } i = 1, \cdots, N
    \end{equation*}
    Similarly, we define $F_{i}(\mathbf{W}, \mathbf{u}) = (f_{\mathbf{W},\mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})^{2}$ and $F(\mathbf{W}, \mathbf{u}) = \sum_{i=1}^{N} F_{i}(\mathbf{W}, \mathbf{u})$. Our problem now is to solve:
    \begin{equation*}
        \min_{\substack{\mathbf{W} \in \mathbb{R}^{p \times n}\\\mathbf{u} \in \mathbb{R}^{p}}} \sum_{i=1}^{N} (f_{\mathbf{W},\mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})^{2} \iff \min_{\substack{\mathbf{W} \in \mathbb{R}^{p \times n}\\\mathbf{u} \in \mathbb{R}^{p}}} F(\mathbf{W}, \mathbf{u}) \iff \min_{\substack{\mathbf{W} \in \mathbb{R}^{p \times n}\\\mathbf{u} \in \mathbb{R}^{p}}} \sum_{i=1}^{N} F_{i}(\mathbf{W}, \mathbf{u}).
    \end{equation*}
    Now, we need $\nabla_{\mathbf{W}^{(\ell)}} F_{i}(\mathbf{W}, \mathbf{u})$ for $\ell = 1, \cdots, L$ and $\nabla_{\mathbf{u}} F_{i}(\mathbf{W}, \mathbf{u})$. Define:
    \begin{align*}
        \mathbf{v}^{(0)} &= \mathbf{x}^{(i)} & \mathbf{v}^{(i)} &= \sigma(\mathbf{W}^{(i)}\mathbf{v}^{(i-1)}), & &\text{for } i = 1, \cdots, L\\
        \mathbf{s}^{(L)} &= \sigma & \mathbf{s}^{(j)} &= \sigma \circ \mathbf{W}^{(L)} \circ \cdots \circ \mathbf{W}^{(j+1)} \circ \sigma = \mathbf{s}^{(j+1)} \circ \mathbf{W}^{(j+1)} \circ \sigma, & &\text{for } j = 0, \cdots, L-1
    \end{align*}
    Therefore, we can rewrite $f_{\mathbf{W}, \mathbf{u}}$ into:
    \begin{equation*}
        f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) = \inprod{\mathbf{u}}{\mathbf{s}^{(\ell)}(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)})}, \qquad \text{for } \ell = 1, \cdots, L.
    \end{equation*}
    It is similar to single-layer neural network for $\nabla_{\mathbf{u}} F_{i}(\mathbf{W}, \mathbf{u})$. We have:
    \begin{equation*}
        \nabla_{\mathbf{u}} F_{i}(\mathbf{W}, \mathbf{u}) = 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i}) \cdot \nabla_{\mathbf{u}}\left(\inprod{\mathbf{u}}{\mathbf{v}^{(L)}}\right) = 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})\mathbf{v}^{(L)}
    \end{equation*}
    However, for $\ell = 1, \cdots, L$, for $\mathbf{V}^{(\ell)} \in \mathbb{R}^{n_{\ell} \times n_{\ell-1}}$,
    \begin{align*}
        D_{\mathbf{W}^{(\ell)}} F_{i}(\mathbf{W}, \mathbf{u})(\mathbf{V}^{(\ell)}) &= 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i}) \cdot D_{\mathbf{W}^{(\ell)}}\left(\inprod{\mathbf{u}}{\mathbf{s}^{(\ell)}(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)}))}\right)(\mathbf{V}^{(\ell)})\\
        &= 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})\inprod{\mathbf{u}}{D(\mathbf{s}^{(\ell)}(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)}))(\mathbf{V}^{(\ell)})}\\
        &= 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})\inprod{\mathbf{u}}{D \mathbf{s}^{(\ell)}(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)})\mathbf{V}^{(\ell)}\mathbf{v}^{(\ell-1)}}\\
        &= \inprod{2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})(D \mathbf{s}^{(\ell)}(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)}))^{T}\mathbf{u}(\mathbf{v}^{(\ell-1)})^{T}}{\mathbf{V}^{(\ell)}}
    \end{align*}
    Denote $\mathbf{M}^{(\ell)} = D \mathbf{s}^{(\ell)}(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)})$. Then,
    \begin{equation*}
        \nabla_{\mathbf{W}^{(\ell)}} F_{i}(\mathbf{W}, \mathbf{u}) = 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})(\mathbf{M}^{(\ell)})^{T}\mathbf{u}(\mathbf{v}^{(\ell-1)})^{T}
    \end{equation*}
    \newpage

    How do we find $\mathbf{M}^{(\ell)}$? We can use recursion. By the chain rule,
    \begin{align*}
        D \mathbf{s}^{(\ell)}(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)}) &= D \mathbf{s}^{(\ell+1)}(\mathbf{W}^{(\ell+1)}\sigma(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)})) \circ D \mathbf{W}^{(\ell+1)}(\sigma(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)})) \circ D \sigma(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)})\\
        &= D \mathbf{s}^{(\ell+1)}(\mathbf{W}^{(\ell+1)}\mathbf{v}^{(\ell)}) \circ D \mathbf{W}^{(\ell+1)}(\mathbf{v}^{(\ell)}) \circ D \sigma(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)}),\\
        \mathbf{M}^{(\ell)} &= \mathbf{M}^{(\ell+1)}\mathbf{W}^{(\ell+1)}\diag(\sigma'(\mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)})).
    \end{align*}
    ...
    \begin{equation*}
        \mathbf{a} \odot \mathbf{b} = \begin{pmatrix}
            ...
            \vdots\\
            a_{n}b_{n}
        \end{pmatrix} = \diag(\mathbf{a})\mathbf{b}.
    \end{equation*}
    For simplicity, we combine some of the matrices as follows:
    \begin{align*}
        \mathbf{z}^{(\ell)} &= (\mathbf{M}^{(\ell)})^{T}\mathbf{u}, & \mathbf{p}^{(\ell)} &= \mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)}.
    \end{align*}
    Expanding $\mathbf{z}^{(\ell)}$, we have:
    \begin{align*}
        \mathbf{z}^{(\ell)} &= (\mathbf{M}^{(\ell)})^{T}\mathbf{u}\\
        &= \diag(\sigma'(\mathbf{p}^{(\ell)}))(\mathbf{W}^{(\ell+1)})^{T}(\mathbf{M}^{(\ell+1)})^{T}\mathbf{u}\\
        &= \sigma'(\mathbf{p}^{(\ell)}) \odot (\mathbf{W}^{(\ell+1)})^{T}\mathbf{z}^{(\ell+1)},\\
        \nabla_{\mathbf{W}^{(\ell)}} F_{i}(\mathbf{W}, \mathbf{u}) &= 2(f_{\mathbf{W}, \mathbf{u}}(\mathbf{x}^{(i)}) - y_{i})\mathbf{z}^{(\ell)}(\mathbf{v}^{(\ell-1)})^{T}.
    \end{align*}
    We now have the following gradient descent algorithm:
    \begin{algorithm}
    \caption{Gradient Descent for Deep Neural Network}
    \begin{algorithmic}
    \Require{$\alpha_{k} > 0$}
    \State{$\nabla_{\mathbf{u}} F(\mathbf{W}, \mathbf{u}) \gets \mathbf{0}$}
    \For{$\ell = L-1, \dots, 1$}
        \State{$\nabla_{\mathbf{W}^{(\ell)}} F(\mathbf{W}, \mathbf{u}) \gets \mathbf{O}$}
    \EndFor
    \For{$i = 1, \dots, N$}
        \State{$\mathbf{v}^{(0)} \gets \mathbf{x}^{(i)}$}
        \For{$\ell = 1, 2, \dots, L$}
            \State{$\mathbf{p}^{(\ell)} \gets \mathbf{W}^{(\ell)}\mathbf{v}^{(\ell-1)}$}
            \State{$\mathbf{v}^{(\ell)} \gets \sigma(\mathbf{p}^{(\ell)})$}
        \EndFor
        \State{$a \gets \inprod{\mathbf{u}}{\mathbf{v}^{(L)}}$}
        \State{$\nabla_{\mathbf{u}} F_{i}(\mathbf{W}, \mathbf{u}) \gets 2(a - y_{i})\mathbf{v}^{(L)}$}
        \State{$\nabla_{\mathbf{u}} F(\mathbf{W}, \mathbf{u}) \gets \nabla_{\mathbf{u}} F(\mathbf{W}, \mathbf{u}) + \nabla_{\mathbf{u}} F_{i}(\mathbf{W}, \mathbf{u})$}
        \State{$\mathbf{z}^{(L)} \gets \sigma'(\mathbf{p}^{(L)}) \odot \mathbf{u}$}
        \State{$\nabla_{\mathbf{W}^{(L)}} F_{i}(\mathbf{W}, \mathbf{u}) \gets 2(a - y_{i})\mathbf{z}^{(L)}(\mathbf{v}^{(\ell-1)})^{T}$}
        \State{$\nabla_{\mathbf{W}^{(L)}} F(\mathbf{W}, \mathbf{u}) \gets \nabla_{\mathbf{W}^{(L)}} F(\mathbf{W}, \mathbf{u}) + \nabla_{\mathbf{W}^{(L)}} F_{i}(\mathbf{W}, \mathbf{u})$}
        \For{$\ell = L-1, \dots, 1$}
            \State{$\mathbf{z}^{(\ell)} \gets \sigma'(\mathbf{p}^{(\ell)}) \odot (\mathbf{W}^{(\ell+1)})^{T}\mathbf{z}^{(\ell+1)}$}
            \State{$\nabla_{\mathbf{W}^{(\ell)}} F_{i}(\mathbf{W}, \mathbf{u}) \gets 2(a - y_{i})\mathbf{z}^{(\ell)}(\mathbf{v}^{(\ell-1)})^{T}$}
            \State{$\nabla_{\mathbf{W}^{(\ell)}} F(\mathbf{W}, \mathbf{u}) \gets \nabla_{\mathbf{W}^{(\ell)}} F(\mathbf{W}, \mathbf{u}) + \nabla_{\mathbf{W}^{(\ell)}} F_{i}(\mathbf{W}, \mathbf{u})$}
        \EndFor
    \EndFor
    \State{Update the weights by gradient descent.}
    \end{algorithmic}
    \end{algorithm}
\end{document}